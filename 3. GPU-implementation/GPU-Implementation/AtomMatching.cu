...
cudaError_t atomMatchingWithCuda(float* c, const float *a, const float *b,
	...
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed! for dev_array in compareWithCuda NA = %d NB = %d NMax = %d\n", NA, NB, NMax);
		goto Error;
	}
	...
	//writeResult2File (NA,NB,NMax,milliseconds/1000,"milliseconds",outputFileName);

		...
		const int NA, const int NB, const int NMax, string fileName)
{
	...
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed! for dev_c for %d in scanWithCuda\n", NMax*sizeof(float));
		goto Error;
	}
	...
	calculateSimilarity<<<NMax/1024 +1, 1024>>>(dev_c, dev_a, NA, NB, NMax);
	...
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaDeviceSynchronize returned error code %d after launching addKernel! in scanWithCuda with NB = %d, NMAx = %d\n", cudaStatus, NB, NMax);
		goto Error;
	}
	...
	//printf("NA =%d, NB=%d, NMax=%d time = %.4f\n", NA, NB, NMax, milliseconds);
...
int main()
{
	...
	cudaStatus = calculateSimilarityWithCuda(C, A,NA,NB, NMax, "calculateSimilarity1.txt");
	//print out C for correctness checking
	printf("C[] array is %.2f\n", C[0]);

	testFunction(A, NA, NB, NMax,  1000,10000,1000,"calculateSimilarity1.txt", &calculateSimilarityWithCuda);

...
__global__ void calculateSimilarity2(float* c, const float *a, const int NA,
...
int main()
{
	...
	cudaStatus = calculateSimilarityWithCuda(C, A,NA,NB, NMax,
	 					"../calculateSimilarityTimeResult/calculateSimilarity2.txt");
	//print out C for correctness checking
	printf("C[] array is %.2f\n", C[0]);

	testFunction(A, NA, NB, NMax,  1000,10000,1000,
					 "../calculateSimilarityTimeResult/calculateSimilarity2.txt",
					 &calculateSimilarityWithCuda2);
		...
		const int NA, const int NB, const int NMax, string fileName)
{
	...
	calculateSimilarity2<<<NMax/1024 +1, 1024>>>(dev_c, dev_a, NA, NB, NMax);

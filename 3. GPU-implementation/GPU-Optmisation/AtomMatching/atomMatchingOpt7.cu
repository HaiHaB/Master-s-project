...
int main() {
	...
	cudaStatus = atomMatchingWithCuda7(C, A,B,NA,NB, NMax,0.5f,"atomMatchingWithCuda7.txt");
	...
	testFunction (A, B, NA, NB,  NMax, 500, 4000, 500,0.5f, "atomMatchingOpt7.txt",&atomMatchingWithCuda7);
	testFunction (A, B, NA, NB,  NMax, 10000, 100000, 10000,0.5f, "atomMatchingOpt7.txt",&atomMatchingWithCuda7);
...
__global__ void atomMatchingOpt7(float* c, const float *a,
	...
	//Allocation of float*a into shared memory
  //Note: float*a reprents one single molecule, therefore takes a small memory
...
cudaError_t atomMatchingWithCuda7(float* c, const float *a, const float *b,
	...
	// Launch a kernel on the GPU with one thread for each element.
		atomMatchingOpt7<<<(NA*NB*NMax)/1024+1,1024,NA*NA*sizeof(float)>>>(dev_c, dev_a, dev_b, dev_array, NA, NB, NMax, threshold);

...
__global__ void atomMatchingOpt3(float* c, const float *a,
...
int main() {
    ...
    cudaStatus = atomMatchingWithCuda3(C, A,B,NA,NB, NMax,0.5f,"atomMatchingWithCuda3.txt");
    ...
    cout << endl;
		...
		testFunction (A, B, NA, NB,  NMax, 500, 4000, 500,0.5f, "atomMatchingOpt3.txt",&atomMatchingWithCuda3);
    testFunction (A, B, NA, NB,  NMax, 10000, 100000, 10000,0.5f, "atomMatchingOpt3.txt",&atomMatchingWithCuda3);
...
__global__ void atomMatchingOpt3(float* c, const float *a,
  ...
  // of NB atoms is NA*NB*NMax
	if (tid <NB*NA*NMax) {
		...
		result = 0;
		//Dynamically allocate an bool array for each thread
...
cudaError_t atomMatchingWithCuda3(float* c, const float *a, const float *b,
  ...
  atomMatchingOpt3<<<128, (NA*NB*NMax)/128+1,(NA*NA + NB*NB*NMax)*sizeof(float)>>>(dev_c, dev_a, dev_b, NA, NB, NMax, threshold);

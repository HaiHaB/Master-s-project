...
int main() {
	...
	cudaStatus = atomMatchingWithCuda4(C, A,B,NA,NB, NMax,0.5f,"atomMatchingWithCuda4.txt");
	...
	testFunction (A, B, NA, NB,  NMax, 500, 4000, 500,0.5f, "atomMatchingOpt4.txt",&atomMatchingWithCuda4);
	testFunction (A, B, NA, NB,  NMax, 10000, 100000, 10000,0.5f, "atomMatchingOpt4.txt",&atomMatchingWithCuda4);
...
__global__ void atomMatchingOpt4(float* c, const float *a,
	...
	//Allocation of float*a and float*b into shared memory
	//Note: Due to limited shared memory, kernel call will fall with too large NMax
	extern __shared__ float aShare[];
	if(tid <NA*NA)      aShare[tid]= a[tid];
	__syncthreads();

	extern __shared__ float bShare[];
	if(tid <NB*NB*NMax)   bShare[tid]= b[tid];
	__syncthreads();
...
cudaError_t atomMatchingWithCuda4(float* c, const float *a, const float *b,
	...
	// Launch a kernel on the GPU with one thread for each element.
		atomMatchingOpt4<<<128, (NA*NB*NMax)/128+1,(NA*NA + NB*NB*NMax)*sizeof(float)>>>(dev_c, dev_a, dev_b, NA, NB, NMax, threshold);

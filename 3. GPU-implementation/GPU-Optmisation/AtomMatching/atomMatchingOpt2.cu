...
__global__  void atomMatchingOpt2(float* c, const float *a,
...
cudaError_t atomMatchingWithCuda2(float* c, const float *a, const float *b,
		...
		int main() {
		    ...
		    cudaStatus = atomMatchingWithCuda2(C, A,B,NA,NB, NMax,0.5f,"../MatchingTimeResult/atomMatchingOpt2.txt");

		    for (int i = 0; i< NMax;i++) {
		    for (int j =0; j<NB*NA; j++)  printf("%.2f ", C[NB*NA*i+j]);
		    cout << endl;
		    }
				testFunction (A, B, NA, NB,  NMax, 500, 4000, 500,0.5f, "../MatchingTimeResult/atomMatchingOpt2.txt",&atomMatchingWithCuda2);
		    testFunction (A, B, NA, NB,  NMax, 10000, 100000, 10000,0.5f, "../MatchingTimeResult/atomMatchingOpt2.txt",&atomMatchingWithCuda2);
...
cudaError_t atomMatchingWithCuda2(float* c, const float *a, const float *b,
  ...
  cudaStatus = cudaEventCreate(&start);
	...
	//int block = (NA*NB*NMax)/1024+1;
	atomMatchingOpt2<<<128, (NA*NB*NMax)/128+1>>>(dev_c, dev_a, dev_b, NA, NB, NMax, threshold);
